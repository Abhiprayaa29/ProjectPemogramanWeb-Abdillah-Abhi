        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto align-items-center fw-medium">
                ...
                <?php if(!isset($_SESSION['is_login']) || (isset($_SESSION['user_role']) && $_SESSION['user_role'] == 'client')): ?>
                    ...
                    <li class="nav-item">
                        <a class="nav-link px-3 <?php echo ($currentPage == 'vendor') ? 'active text-primary fw-bold' : ''; ?>" href="vendor.php">Jadi Vendor</a>
                    </li>
                ...
                <?php if(isset($_SESSION['is_login']) && $_SESSION['is_login'] === true): ?>
                    <?php 
                        if($_SESSION['user_role'] == 'vendor'){
                            $sql = "SELECT profile_img FROM vendors WHERE user_id={$_SESSION['user_id']} ";
                        }else{
                            $sql = "SELECT profile_img FROM users WHERE id={$_SESSION['user_id']} ";
                        }

                        $data = mysqli_query($conn, $sql);
                        $user_data_img = mysqli_fetch_assoc($data);
                    ?>
                    <li class="nav-item dropdown ms-lg-3">
                        <a class="nav-link dropdown-toggle d-flex align-items-center bg-light rounded-pill px-3 py-2" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="<?php echo $user_data_img['profile_img']; ?>" 
                                class="rounded-circle mx-auto object-fit-cover border border-white shadow-sm me-1" 
                                width="32" height="32" 
                                alt="User">   
